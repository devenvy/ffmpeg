# FFmpeg 8.1.2 · LGPL — build coverage

_Auto-generated by `scripts/gen-matrix.sh` from FFmpeg n8.1.2's `configure` and the build scripts — do not edit by hand. Back to the [matrix index](../BUILD-MATRIX.md)._
...
| Vulkan (filters + whisper GPU) | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | n/a | n/a |
...
[^tls]: TLS/https backend. OpenSSL is built on Linux/Android; Windows uses SChannel and Apple uses SecureTransport (OS-native `HAVE_*` backends, not FFmpeg library components). All three enable the `https`/`tls` protocols.
