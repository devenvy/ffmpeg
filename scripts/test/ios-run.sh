...
# runner (no slow emulation). Compiles scripts/test/smoke.c against the static
# .a and executes it via simctl.
set -uo pipefail
...
check_smoke_link "${CC} -arch arm64 -mios-simulator-version-min=13.0 -isysroot ${SDK}" \
  ...
  -framework CoreVideo -framework CoreFoundation -framework Security || finish
