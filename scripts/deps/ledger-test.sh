...
cat > "${LEDGER}" <<'JSON'
{
  ...
  "overrides": { "8": { "x265": { "origin": "https://example.test/x265.git", "branch": "stable-8", "reason": "8.x caps libx265 at 3.x" } } }
}
JSON
...
FFMPEG_VERSION=8.1.2 got="$(dep_source x265)";   check "8 override switches ref type" "$got" "$(printf 'https://example.test/x265.git\tbranch\tstable-8')"
