# FFmpeg 8.1.2 · GPLv2 — build coverage
...
_This is the **GPLv2** cell — one of four ({gplv3, gplv2, lgplv3, lgplv2}); the siblings are linked from the [matrix index](README.md). Cells differ in x264/x265 (gpl) vs kvazaar (lgpl) — except the lean `ios-sim-arm64` slice, which carries none of the three — and in the Apache-2.0 dependencies. **v3** adds Vulkan on every RID that has it (all but `ios-sim-arm64`: the lean slice drops libplacebo, so it has no SPIR-V compiler and would register no Vulkan filters), and adds OpenSSL on the platforms that need a bundled TLS backend (Linux, Android, Mac Catalyst); **v2** uses GnuTLS (gpl) or NO TLS (lgpl) on those same platforms and drops Vulkan everywhere. Windows (SChannel) and macOS/iOS (SecureTransport) have TLS in all four cells. Dropping Vulkan sends Whisper to the CPU only where the Vulkan backend was in use: Apple targets use Metal in every cell, and `linux-armhf` and `win-arm64` are CPU in every cell. The per-RID columns below are authoritative; this paragraph is the summary. Every cell ships DYNAMIC libraries — iOS as a dynamic-framework `.xcframework`._
...
| OpenCORE — AMR-NB en/decode |  | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ |
| OpenCORE — AMR-WB decode |  | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ |
| Rubber Band — time-stretch/pitch (GPL) |  | — | — | — | — | — | — | — | — | — | — | — | — | — | — | — |
| Shine — fixed-point MP3 |  | — | — | — | — | — | — | — | — | — | — | — | — | — | — | — |
| TwoLAME — MP2 encode |  | — | — | — | — | — | — | — | — | — | — | — | — | — | — | — |
| VisualOn — AMR-WB encode |  | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ |
...
| Feature | Version | lin-x64 | lin-a64 | lin-hf | musl | musl-a64 | win | win-a64 | osx-x64 | osx-a64 | android | android-x64 | ios | ios-sim | mcat-a64 | mcat-x64 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| fontconfig — font-by-name[^fontconfig] | 2.18.3 | ✓ | ✓ | ✓ | ✓ | ✓ | n/a | n/a | ✓ | ✓ | n/a | n/a | n/a | n/a | n/a | n/a |
...
| Feature | Version | lin-x64 | lin-a64 | lin-hf | musl | musl-a64 | win | win-a64 | osx-x64 | osx-a64 | android | android-x64 | ios | ios-sim | mcat-a64 | mcat-x64 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| AMF (AMD) | c35f613ae | n/a | n/a | n/a | n/a | n/a | ✓ | n/a | n/a | n/a | n/a | n/a | n/a | n/a | n/a | n/a |
...
| QSV (oneVPL) | v2.17.0 | ✓ | — | — | ✓ | — | ✓ | n/a | n/a | n/a | n/a | n/a | n/a | n/a | n/a | n/a |
...
| OpenCL |  | — | — | — | — | — | — | — | — | — | — | — | — | — | — | — |
| OpenMAX IL |  | — | — | — | — | — | n/a | n/a | n/a | n/a | n/a | n/a | n/a | n/a | n/a | n/a |
| QSV (MediaSDK, legacy) |  | — | — | — | — | — | — | n/a | n/a | n/a | n/a | n/a | n/a | n/a | n/a | n/a |
...
| rkmpp |  | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ | ✗ |
| Video4Linux2 |  | — | — | — | — | — | — | — | — | — | — | — | — | — | — | — |
| Vulkan (static ICD) |  | n/a | n/a | n/a | n/a | n/a | n/a | n/a | n/a | n/a | n/a | n/a | — | — | — | — |

[^tls]: TLS/https backend — depends on the license SERIES. v3 builds link OpenSSL 3.x (Apache-2.0, requires --enable-version3) on Linux/Android. The App-Store-safe v2 builds instead use GnuTLS on gpl-2 (its GMP/nettle deps are fine under GPLv2) and DROP TLS ENTIRELY on lgpl-2 (GMP/nettle are never LGPLv2.1 and no other backend is either). Windows uses SChannel and macOS/iOS use SecureTransport (OS-native) in every series. Mac Catalyst is the Apple exception: SecureTransport is unavailable on macabi, so Catalyst follows the Linux/Android ladder instead. All enable the `https`/`tls` protocols EXCEPT lgpl-2 on Linux, Android and Catalyst, which have no TLS.
...
[^svtav1]: Needs 64-bit and a toolchain it cross-compiles under, so it is n/a on armhf, win-arm64, Mac Catalyst and mobile. AV1 encode is still available via libaom everywhere libaom itself is built — which excludes the lean ios-sim-arm64 slice.
