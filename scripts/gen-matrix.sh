...
def render(version, conf, cid):
    ...
    L.append(f"_This is the **{LABEL}** cell — one of four ({{gplv3, gplv2, lgplv3, lgplv2}}); the "
             ...
             f"none of the three — and in the Apache-2.0 dependencies. **v3** adds Vulkan on "
             f"every RID that has it (all but `ios-sim-arm64`: the lean slice drops libplacebo, "
             f"so it has no SPIR-V compiler and would register no Vulkan filters), and "
